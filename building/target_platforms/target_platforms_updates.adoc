...
. <<determine-if-dependency-is-mirrored,Determine if dependency is mirrored>>
.. <<create-a-build.xml-to-mirror-3rd-party-repository,Create a build.xml to mirror 3rd-party repository>>
. <<create-and-submit-target-platform-change,Create and submit target-platform change>>
.. <<fetch-sources,Fetch sources>> 
.. <<build-target-platforms-locally,build locally>>
.. <<update-the-target-files,Update .target files>>
...
== Determine if dependency is mirrored

The next step to requesting a new bundle be added to the target platform is to determine whether the bundle is already mirrored. JBoss Tools mirrors our requirements at http://download.jboss.org/jbosstools/updates/requirements/

If your dependency is hosted at eclipse.org, there's a good chance that the bits are already mirrored. You will want to look in http://download.jboss.org/jbosstools/updates/requirements/mars or substituting the name of whichever release is currently being targeted in place of mars (luna, mars, neon, etc). 

Some projects hosted at eclipse.org may not be present in this location for any number of reasons, or their versions may not be the version you require. In that case, your requirement may have its own folder in http://download.jboss.org/jbosstools/updates/requirements. One example is the `Targeted Management` plugins, which have their own folder named `tm` in the requirements folder of download.jboss.org.  You may need to be creative while looking for your dependencies if your requirement is a subproject of some other larger project. Unfortunately, there's no easy way to determine this definitively, and you may need to work with the build team to locate your requirement. 

If your requirement is not already mirrored, you will need to create a build.xml to mirror the 3rd-party repository. If your dependency is already mirrored, you can skip this next section, and proceed directly to adding it to the target platform. 

=== Create a build.xml to mirror 3rd-party repository

You've now determined that your requirement is not yet mirrored on jboss.org, and so some extra work must be done to make it available to our target platform. 

First, clone the http://github.com/jbosstools/jbosstools-download.jboss.org[jbosstools-download.jboss.org] Git repository. 

Then browse to the `jbosstools/updates/requirements` folder. This is where you'll create a new folder for your requirement, and place a build.xml inside of it. 

The build.xml you'll create is essentially a wrapper to updates/requirements/generic/build.xml, which does the majority of the work.  The generic build.xml is a script that uses the `<p2.mirror>` https://wiki.eclipse.org/Equinox/p2/Ant_Tasks#Mirror_Task[ant task] to produce a mirror of one or more update sites, optionally selecting which IUs to mirror, and then removing references to any third-party update sites so that the resulting mirror is self-contained.  
...
The easiest way to get a good `build.xml` is to copy a `build.xml` from another 3rd-party dependency in the requirements folder, such as Thym, and adapt the various properties (URL, version...) to match your request. Then, commit your change and submit the change as a pull request. Don't forget to attach it to the Jira.

An admin of the repository should review it shortly thereafter. In case it takes too long, feel free to ping people on the JIRA, IRC, or the mailing lists, to make sure your change is reviewed and applied promptly.

== Create and submit target-platform change

=== Fetch sources

The following instructions cover the minimal edits needed for a review (a final Pull request needs updates to both jbosstools and devstudio target platform as needed)

```bash
git clone git://github.com:jbosstools/jbosstools-target-platforms.git # or git clone git@github.com:jbosstools/jbosstools-target-platforms.git
...
```bash
git clone git://github.com:jbosstools/jbosstools-integration-stack.git # or git clone git@github.com:jbosstools/jbosstools-integration-stack.git
...
To build the target platform, you require specific versions of Java and Maven. Also, there is some Maven setup required. 
...
If you just want to check if things compile and build properly, you can run:
